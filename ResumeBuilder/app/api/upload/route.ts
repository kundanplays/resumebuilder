      ...
      if (service.isTarballUpload) {
        ...
        try {
          ...
          response = await fetch(actualFetchUrl, {
            method: 'POST',
            body: formData as any, // Cast to resolve type mismatch between formdata-node and native FormData
  ...
  try {
    ...
    if (!mimeType || mimeType === 'application/octet-stream') {
      ...
      if (file.name.endsWith('.pdf')) {
        ...
        // Word documents are NOT supported by Gemini API
        return NextResponse.json(
          { 
            error: "Word documents (.doc/.docx) are not currently supported. Please convert your resume to PDF format and try again.",
            supportedFormats: ["PDF (.pdf)", "Plain text (.txt)"]
          },
          { status: 400 }
        );
        ...
        // For other unknown file types, return an error
        return NextResponse.json(
          { 
            error: "Unsupported file format. Please upload your resume in PDF or plain text format.",
            supportedFormats: ["PDF (.pdf)", "Plain text (.txt)"]
          },
          { status: 400 }
        );
    ...
    // Additional validation for MIME types that Gemini API supports
    ...
    if (!supportedMimeTypes.includes(mimeType)) {
      return NextResponse.json(
        { 
          error: `File type "${mimeType}" is not supported. Please upload your resume in PDF or plain text format.`,
          supportedFormats: ["PDF (.pdf)", "Plain text (.txt)"]
        },
        { status: 400 }
      );
...
**CRITICAL EDUCATION INSTRUCTION**: When extracting education information, you MUST preserve the exact year format as it appears in the original resume. If the resume shows "2024 - Present", use exactly that. If it shows "2000-2005", use exactly that. If NO year is mentioned for any education entry, do NOT add or fabricate years - leave the DurationOrYear field as null or omit it.

**CRITICAL COMPANY AND NUMBERS INSTRUCTION**: 
1. **Company Names**: ALWAYS extract the EXACT company name from the original resume. NEVER use placeholders like "[Company Name]" or generic terms like "Company". If a company name is mentioned in the resume, use it exactly as written.
2. **Numbers & Percentages**: NEVER use placeholder text like "[specify percentage]", "[Number]", "[X%]", or "[achieve X% growth]". Instead:
   - If the original resume contains specific numbers/percentages, extract and use them EXACTLY
...
\`\`\`json
{
  ...
  "Education": [
    {
      ...
      //Critical : If resume has multiple education in different year mentioned, provide only the specific year for each education.
      "DurationOrYear": "IMPORTANT: Extract the EXACT year/duration format as mentioned in the original resume. If the resume shows '2024 - Present', use that exact format. If it shows 'Graduated 2018', use that. If it shows '2000-2005', use that. If NO year is mentioned for an education entry, set this field to null or omit it entirely. DO NOT fabricate or assume years that are not explicitly stated in the resume.",
    ...
    // List in reverse chronological order ONLY if years are available. If no years are present, list in the order they appear in the original resume.
  ],
  ...
  "Experience": [
    ...
    // --- If '${experienceLevel}' indicates an EXPERIENCED candidate (e.g., not 'entry-level', 'fresher', 'intern', '0 years'): ---
    {
      ...
      "CompanyName": "CRITICAL: Extract the EXACT company name from the original resume. NEVER use '[Company Name]' or generic placeholders. Use the actual company name as written in the resume.",
      "Location": "City, State (Optional)",
      ...
      "ImpactMade": [
        // CRITICAL: Quantify achievements with specific, realistic numbers. DO NOT use placeholders like '[X%]' or '[Number]'.
        // If the original resume contains specific numbers/percentages, extract and use them EXACTLY.
        // If creating new achievements, use realistic, specific numbers appropriate for the role and experience level.
        // Examples: "Achieved a 15% improvement in lead conversion rates", "Successfully mentored and developed 5 team members", "Contributed to $250K revenue growth"
        // Reframe impacts to show value for a '${targetJobRole}'.
        "Tailored/Created Quantifiable Impact with SPECIFIC numbers (no placeholders)...",
        "Tailored/Created Quantifiable Impact with SPECIFIC numbers (no placeholders)..."
      ]
    }
    ...
    //   "CompanyName": "EXACT company name from resume, never use placeholders",
    //   "Location": "City, State (Optional)",
    ...
    //     "Key learning outcome or specific contribution with SPECIFIC numbers where appropriate, e.g., 'Assisted in market research that contributed to the identification of 3 new potential customer segments.'"
    //   ]
    // }
  ],
  "Projects": [
...
- NEVER use placeholder text like "[Company Name]", "[Number]", "[specify percentage]", "[X%]", or any brackets with placeholder content
- ALWAYS extract EXACT company names from the original resume
...
\\end{document}`;
    ...
    try {
      ...
      console.log("✅ Professional Blue compiled successfully");
    } catch (error) {
      console.error("❌ Professional Blue failed completely:", error);
    }
    ...
    try {
      ...
      console.log("✅ Modern Green compiled successfully");
    } catch (error) {
      console.error("❌ Modern Green failed completely:", error);
    }
    ...
    try {
      ...
      console.log("✅ Compact Classic compiled successfully");
    } catch (error) {
      console.error("❌ Compact Classic failed completely:", error);
    }
